...
def build_filename_from_subject(subject: str, year_mode: str = "current") -> str | None:
    ...
    # OS/OC + N°ORDEN
    m_doc   = re.search(r"\bO\.?\s*([SC])\.?\s*(\d{1,10})\b", s)
    ...
    if year_mode == "current":
    ...
    elif year_mode in {"subject", "subject_plus1"}:
        ...
        raise ValueError(f"year_mode inválido: {year_mode}")
