...
LOGOS_DIR = os.path.join(os.path.dirname(__file__), "assets", "logo")
...
def cargar_columnas_excel():
    ...
    try:
        ...
        messagebox.showinfo(
            "Listo", "Columnas cargadas. Revisa/ajusta el mapeo y luego envía."
        )
...
def actualizar_tabla_preview():
    ...
    for idx, (_, row) in enumerate(preview_df.iterrows(), start=1):
        v_n = (
            _safe_cell_to_text(row[col_n])
            if col_n and col_n in preview_df.columns
            else ""
        )
        v_c = (
            _safe_cell_to_text(row[col_c])
            if col_c and col_c in preview_df.columns
            else ""
        )
        v_a = (
            _safe_cell_to_text(row[col_a])
            if col_a and col_a in preview_df.columns
            else ""
        )
...
def insertar_logo():
    ...
    file_path = filedialog.askopenfilename(
        title="Seleccionar logo", filetypes=[("Imágenes", "*.png;*.jpg;*.jpeg;*.gif")]
    )
    if not file_path:
        return
...
def _set_ui_enviando(enviando: bool):
    ...
    reporte_entry.configure(
        state=state_inputs if (gen_excel_var.get() or gen_pdf_var.get()) else "disabled"
    )
    btn_reporte.configure(
        state=state_inputs if (gen_excel_var.get() or gen_pdf_var.get()) else "disabled"
    )
...
def abrir_modal_envio_y_ejecutar():
    ...
    if not col_nombre or not col_correo or not col_archivo:
        messagebox.showerror(
            "Error", "Selecciona las 3 columnas: Nombre, Correo y NombreArchivo."
        )
        return
    ...
    try:
        ...
        messagebox.showerror(
            "Error", "El 'Máximo por ejecución' debe ser un entero > 0."
        )
        return
    ...
    runner_kwargs = dict(
        ...
        mensaje=mensaje,
        ruta_base_reporte=(
            reporte_entry.get().strip()
            if (gen_excel_var.get() or gen_pdf_var.get())
            else None
        ),
...
# Excel
ctk.CTkLabel(left_content, text="Archivo Excel", font=FONT_LABEL).pack(
    anchor="w", pady=(0, PADY_SM)
)
excel_entry = ctk.CTkEntry(left_content)
excel_entry.pack(fill="x", pady=(0, PADY_MD))
btn_excel = ctk.CTkButton(
    left_content, text="Seleccionar Excel", command=seleccionar_excel, font=FONT_BUTTON
)
...
ctk.CTkLabel(
    left_content, text=f"Vista previa ({PREVIEW_N} filas)", font=FONT_LABEL
).pack(anchor="w", pady=(0, PADY_SM))
...
preview_tree = ttk.Treeview(
    frame_table, columns=columns, show="headings", height=PREVIEW_N
)
...
# Carpeta
ctk.CTkLabel(
    left_content, text="Carpeta donde están TODOS los archivos", font=FONT_LABEL
).pack(anchor="w", pady=(0, PADY_SM))
...
# Asunto
ctk.CTkLabel(right_content, text="Asunto", font=FONT_LABEL).pack(
    anchor="w", pady=(0, PADY_SM)
)
...
# Mensaje
ctk.CTkLabel(right_content, text="Mensaje", font=FONT_LABEL).pack(
    anchor="w", pady=(0, PADY_SM)
)
...
ctk.CTkLabel(frame_envio, text="Máximo por ejecución", font=FONT_LABEL).grid(
...
ctk.CTkLabel(frame_envio, text="Delay (segundos) entre envíos", font=FONT_LABEL).grid(
...
# Sección Reporte
ctk.CTkLabel(right_content, text="Informe / Registro", font=FONT_LABEL).pack(
    anchor="w", pady=(0, PADY_SM)
)
...
ctk.CTkLabel(
    right_content, text="Ruta base del informe (sin extensión)", font=FONT_LABEL
).pack(anchor="w", pady=(0, PADY_SM))
...
ctk.CTkLabel(right_content, text="Título del reporte", font=FONT_LABEL).pack(
    anchor="w", pady=(0, PADY_SM)
)
